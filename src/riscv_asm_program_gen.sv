...
class riscv_asm_program_gen extends uvm_object;
  ...
  virtual function void gen_trap_jmp(ref string instr[$], input int hart, input string mode, input exception_cause_t cause, input string label);
    string skip_label = $sformatf("h%0d_%0s_chk_n_trp_%0d", hart, mode, cause);

    instr = {instr,
      $sformatf("li x%0d, 0x%0x", cfg.gpr[1], cause),
      $sformatf("bne x%0d, x%0d, %0s", cfg.gpr[0], cfg.gpr[1], skip_label),
      $sformatf("  la  x%0d, %0s%0s_handler", cfg.gpr[1], hart_prefix(hart), label),
      $sformatf("  jr  x%0d", cfg.gpr[1]),
      $sformatf("%s:", skip_label)
      };
  endfunction
  ...
  virtual function void gen_trap_handler_section(int hart,
    ...
    // Breakpoint
    gen_trap_jmp(instr, hart, mode,BREAKPOINT,"ebreak");
    // Check if it's an ECALL exception. Jump to ECALL exception handler
    gen_trap_jmp(instr, hart, mode,ECALL_UMODE,"ecall");
    gen_trap_jmp(instr, hart, mode,ECALL_SMODE,"ecall");
    gen_trap_jmp(instr, hart, mode,ECALL_MMODE,"ecall");
    // Page table fault or access fault conditions
    gen_trap_jmp(instr, hart, mode,INSTRUCTION_ACCESS_FAULT,"instr_fault");
    gen_trap_jmp(instr, hart, mode,LOAD_ACCESS_FAULT,"load_fault");
    gen_trap_jmp(instr, hart, mode,STORE_AMO_ACCESS_FAULT,"store_fault");
    gen_trap_jmp(instr, hart, mode,INSTRUCTION_PAGE_FAULT,"pt_fault");
    gen_trap_jmp(instr, hart, mode,LOAD_PAGE_FAULT,"pt_fault");
    gen_trap_jmp(instr, hart, mode,STORE_AMO_PAGE_FAULT,"pt_fault");
    // Illegal instruction exception
    gen_trap_jmp(instr, hart, mode,ILLEGAL_INSTRUCTION,"illegal_instr");
  ...
  virtual function void gen_interrupt_vector_table(int              hart,
    ...
    for (int i = 1; i < max_interrupt_vector_num; i++) begin
      ...
      intr_handler = {intr_handler,
                      $sformatf("la x%0d, %0s%0smode_intr_handler", cfg.gpr[0], hart_prefix(hart), mode),
                      $sformatf("jr x%0d", cfg.gpr[0]),
                      $sformatf("1: la x%0d, test_done", cfg.scratch_reg),
